...
type CatalogDetailItem = CatalogListItem & {
  media?: {
    thumbnail?: CatalogMediaItem | null;
...
type CatalogDetailResponse = CatalogDetailItem;

const mapCatalogBrand = (brand?: CatalogBrand): Brand | undefined => {
...
const mapCatalogCategory = (category: CatalogCategory): Category => ({
  id: category.id,
  name: category.name,
  slug: category.slug,
  isActive: true,
...
const mapCatalogTag = (tag: CatalogTag): Tag => ({
  id: tag.id,
  name: tag.name,
  slug: tag.slug,
  isActive: true,
...
const mapCatalogItemToProductListItem = (
...
const normalizeProductsParams = (params: ProductsQueryParams): Record<string, unknown> => {
